...
let index = 0;
let typoIndex = 0;
const typos = ["Praht", "Ngaar", "Pratam"]; // Intentional errors
let isCorrecting = false;
let currentText = "";

function type() {
  let randomTypo = Math.random() < 0.1; // 10% chance to make a typo
  if (!isCorrecting && randomTypo && typoIndex < typos.length && currentText === text.substring(0, index)) {
    currentText += typos[typoIndex][index - currentText.length];
    typoIndex++;
  } else {
  ...
  if (index > text.length) {
    ...
    typoIndex = 0;
    currentText = "";
    setTimeout(type, 2000); // Restart after a pause
  } else {
    let delay = isCorrecting ? 100 : 200; // Faster correction speed
    setTimeout(type, delay);
  }

  // Check if need to correct
  if (currentText !== text.substring(0, index)) {
    isCorrecting = true;
    index--;
  } else {
    isCorrecting = false;
...
document.addEventListener('DOMContentLoaded', function () {
  const target = document.querySelector('.st-hero.st-style1 .st-hero-text h1');
  const text = "Pratham Nagar";
  let index = 0;
  let currentText = "";
  let isTyping = true;
  ...
  function erase() {
      if (!isTyping) {
          if (currentText.length > 0) {
              ...
              isTyping = true;
              index = 0;
              setTimeout(type, 2000); // Wait before typing again
          }
      }
  }
